...
SDLWindow::SDLWindow(const std::string& name, const WindowSize size, const int top, const int left, const uint32_t flags)
{
    SDL_Init(SDL_INIT_VIDEO | SDL_INIT_EVENTS);
    m_SDLHandle = SDL_CreateWindow(name.data(), top, left, size.width, size.height, flags | SDL_WINDOW_VULKAN);
    toggleMouseCapture();
...
void SDLWindow::createSurface(const VkInstance instance)
{
    ...
    Logger::print("SDLHandle surface created", Logger::INFO);
}
...
void SDLWindow::free()
{
    Logger::print("Destroying and closing window...", Logger::INFO);
    if (m_surface != nullptr)
    {
...
{
    Logger::print("Shutting down imgui SDL2 backend...", Logger::INFO);
